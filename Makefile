...
build-job:
	...
	docker build -q -f deploy/Dockerfile -t osb-node-img $(CURDIR)
...
cleanup-build:
	...
	docker container stop osb-container-catalog > /dev/null || $(MAKE) skip-message
	docker container rm osb-container-catalog > /dev/null || $(MAKE) skip-message
	docker container stop osb-container-namespace > /dev/null || $(MAKE) skip-message
	docker container rm osb-container-namespace > /dev/null || $(MAKE) skip-message
	docker container stop osb-container-build > /dev/null || $(MAKE) skip-message
	docker container rm osb-container-build > /dev/null || $(MAKE) skip-message
	@echo "Done."

cleanup-deploy-ce:
	@echo  ......cleaning up after ce deploy
	docker container stop osb-container-deploy-ce > /dev/null || $(MAKE) skip-message
	docker container rm osb-container-deploy-ce > /dev/null || $(MAKE) skip-message
	@echo "Done."
